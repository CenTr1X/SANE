...
{
  printf("recv someip msg on method 2, requestid:%d, data:%s, content_length:%d\n", requestId,req->data, req->length);
...
{
  printf("recv sensor fire msg , requestid:%d, data:%s, content_length:%d\n", requestId,req->data, req->length);
  printf("Tire No.%d ", *(req->data));
  printf("pressure:%f", strtod(req->data+1,NULL));
  //update_tire_pressure(*(req->data), strtod(req->data+1,NULL));
  return E_OK;
}